...
class VozScraper(BaseScraper):
    ...
    async def fetch_articles(self) -> List[ScrapedArticle]:
        """
        Fetch thread titles from configured Voz sections.
        ...
        for section_key in self.sections:
            ...
            try:
                section_articles = await self._fetch_section(section_key, url)
            except Exception as exc:
                logger.error(
            ...
            # Rate-limit: 1 second between sections
            await asyncio.sleep(1.0)

        logger.info(
            "voz: fetched %d articles across %d sections",
